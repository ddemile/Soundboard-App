    ...
    return (
        <div>
            <CategoryContextMenu category={props}>
                <span className='ml-2.5 text-left flex items-center gap-1 font-semibold'><Icon /> {name} <button onClick={(e) => onExpandToggle(e, name)} className='bg-transparent p-0 border-none outline-hidden focus:outline-hidden'>{expanded ? <MdOutlineKeyboardArrowDown /> : <MdOutlineKeyboardArrowRight />}</button></span>
            </CategoryContextMenu>
            {expanded && <ul className='mb-2 grid mx-2.5 mt-1 gap-2.5 grid-cols-150 list-none'>
                {sounds.map((sound: SoundEntry) =>
                    <SoundContextMenu key={sound.id} sound={sound} category={props}>
                        <li className='*:col-start-1 *:row-start-1 grid rounded-lg overflow-hidden group h-10'>
                            ...
                            <div className='bg-black/0 opacity-0 group-hover:bg-black/25 dark:group-hover:bg-black/50 group-hover:opacity-100 w-full flex items-center justify-between px-1 transition-opacity'>
                                <button className='border-none outline-hidden focus:outline-hidden bg-transparent rounded-full aspect-square p-0 *:text-xl' onClick={() => handleSoundPreview(sound)}>
                                    <IoVolumeMedium />
                                </button>
                                <button className='border-none outline-hidden focus:outline-hidden bg-transparent rounded-full aspect-square p-0 *:text-3xl' onClick={() => play(sound)}>
                                    <BsPlayFill />
                                </button>
                                <button className='border-none outline-hidden focus:outline-hidden bg-transparent rounded-full aspect-square p-0 *:text-md' onClick={() => handleFavorite(sound)}>
                ...
                <li className='bg-neutral-100 dark:bg-main max-w-[200px] flex gap-1 items-center p-1 rounded-lg h-10 justify-center'>
                    <button className='bg-neutral-300 dark:bg-stone-900 rounded-full aspect-square p-1 border-none outline-hidden focus:outline-hidden' onClick={(e) => {
                        e.preventDefault()
