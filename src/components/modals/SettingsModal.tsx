...
export default function SettingsModal() {
  ...
  return (
    <Modal isOpen={isOpen} onRequestClose={() => setIsOpen(false)} className='rounded-lg m-4 w-auto h-auto p-0 bg-white dark:bg-[#303031] flex-row overflow-clip grow' overlayClassName="flex justify-normal items-stretch">
      <nav className='border-white border-opacity-20 border-r-2 h-full bg-neutral-200 dark:bg-[#232324] flex p-14 pr-0 flex-col'>
        ...
        <ul className='flex flex-col gap-0.5 text-left mr-1 w-48'>
          ...
          <li className={`p-1.5 px-2 hover:bg-neutral-300 dark:hover:bg-opacity-5 dark:hover:bg-white cursor-pointer rounded-md ${!(websocket.auth as any)?.webInterfaceCode && "hover:cursor-not-allowed"}`} onClick={() => {
